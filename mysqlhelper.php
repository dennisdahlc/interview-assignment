...
class MySQLHelper
{
    ...
    public static function performSelectOperation($event)
    {
        $SQL = self::constructSQL(self::$SQLForSelection, $event);
    ...
    public static function performDeleteOperation($isResultAddedToFile, $event)
    {
        if($isResultAddedToFile)
        {
            $sqlForRemoval = self::constructSQL(self::$SQLForDeletion, $event);
    ...
    private static function constructSQL($SQLOperation, $event)
    {
        $expression = self::constructExpression($event);
    ...
    private static function constructExpression($event)
    {
        $columnName = self::extractColumnName($event);

        $operator = self::extractOperatorName($event);
        
        $value = self::extractValue($event);
    ...
    private static function extractColumnName($event)
    {
        $columnIndex = self::convertInt($event->field);
        
        self::assertIsInteger($columnIndex, "Field is an integer");
        self::assertTrue($columnIndex >= 0 && $columnIndex < count(self::$SQLColumnNames), "ColumnIndex is within array bounds", $columnIndex);
        
        $columnName = self::$SQLColumnNames[$columnIndex];

        return $columnName;        
    }
    
    private static function extractOperatorName($event)
    {
        $operatorIndex = self::convertInt($event->operator);
        
        self::assertIsInteger($operatorIndex, "Operator is an integer");
        self::assertTrue($operatorIndex >= 0 && $operatorIndex < count(self::$SQLColumnNames), "OperatorIndex is within array bounds", $operatorIndex);
        
        $operator = self::$SQLOperators[$operatorIndex];

        return $operator;        
    }
    
    private static function extractValue($event)
    {
        $value = $event->value;
        
        $value = self::$conn->real_escape_string($value); //sanitize value for sql
        
        self::assertIsString($value, "Value is a string");
    ...
    private static function convertInt($value)
    {
        $result = null;
        ...
        {
            self::terminateWithErrorMessage("Convertion of int failed", $exc->getTraceAsString());
        }
        
        return $result;
    }
    
    private static function assertIsString($value, $assertionDescription)
    {
        self::assertTrue(is_string($value), $assertionDescription, $value);
    }
    
    private static function assertIsInteger($value, $assertionDescription)
    {
        self::assertTrue(is_int($value), $assertionDescription, $value);
    }
    
    private static function assertTrue($assertion, $assertionDescription, $value)
    {
        if(!$assertion)
        {
            self::terminateWithErrorMessageAndError("Assertion " . $assertionDescription . " failed with value: " . $value);
        }
    }
    
    private static function terminateWithErrorMessage($errorMessage)
    {
        self::terminateWithErrorMessageAndError($errorMessage, self::$conn->error);
    }
