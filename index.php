...
class MySQLHelper
{
    ...
    {
        $sqlForCreationOfTable = "CREATE TABLE IF NOT EXISTS users (
        ...
        for ($index = 0; $index < count($sqlForInsertingDatas); $index++) 
        {
            self::executeSql($sqlForInsertingDatas[$index], "Failed to insert data");
        } 
    }
    
    public static function executeSql($sql, $errorMessage)
    {
        if (self::$conn->query($sql) === FALSE)
        {
            self::terminateWithError($errorMessage);
        } 
    }
    ...
    private static function terminateWithErrorMessageAndError($errorMessage, $error)
    {
        self::closeConnection();
        die($errorMessage . " with error: " . $error);
    }
